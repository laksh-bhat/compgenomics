package project.cs439.filter.bloom;
...
// Bloom Filter based on http://dark-code.bulix.org/1epygx-65873
// For production purposes use Google Guava Bloom Filter
...
public class BloomFilter<E> implements Serializable {
    ...
    private int maxNoOfFilterElements;
    private int numberOfAddedElements;

    static final Charset charset = Charset.forName("UTF-8"); // encoding used for storing hash values as strings
    static final String hashName = "MD5";  // Murmur Implementation has some issues
     ...
     * The optimal number of hash functions (noOfHashFunctions) is estimated from the total size of the Bloom Filter
     ...
     * The number of bits per element and the number of hash functions is estimated
    ...
    public BloomFilter(double falsePositiveProbability, int expectedNumberOfElements) {
        this(Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))) / Math.log(2), // c = noOfHashFunctions / ln(2)
             expectedNumberOfElements,
             (int)Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2)))); // noOfHashFunctions = ceil(-log_2(false prob.))
    }
    ...
    /**
     * Generates a hash based on the contents of a string.
     *
    ...
    public boolean equals(Object obj) {
        ...
        if (this.maxNoOfFilterElements != other.maxNoOfFilterElements) {
            return false;
        }
        if (this.noOfHashFunctions != other.noOfHashFunctions) {
            return false;
        }
    ...
    public int hashCode() {
        ...
        hash = 61 * hash + this.maxNoOfFilterElements;
        hash = 61 * hash + this.bitSetSize;
        hash = 61 * hash + this.noOfHashFunctions;
        return hash;
    }
    ...
    public double expectedFalsePositiveProbability() {
        return getFalsePositiveProbability(maxNoOfFilterElements);
    }
    ...
    public double getFalsePositiveProbability(double numberOfElements) {
        // (1 - e^(-noOfHashFunctions * n / m)) ^ noOfHashFunctions
        return Math.pow((1 - Math.exp(-noOfHashFunctions * (double) numberOfElements
                                              / (double) bitSetSize)), noOfHashFunctions);
     ...
     *
     * @return optimal noOfHashFunctions.
     */
    ...
    public void add(byte[] bytes) {
        int[] hashes = createHashes(bytes, noOfHashFunctions);
        for (int hash : hashes)
